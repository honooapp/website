...
class jqgrid
{
	 ...
	 */	
	function get_dependent_dropdown($sql)
	{
		$select = array();
		...
		for($i=0;$i<count($cols);$i++)
		{
			...
			if (is_array($cols[$i]["editoptions"]["onchange"]))
			{
				...
				if ($_POST["src"] == $cols[$i]["name"] && $_POST["return"] == "select")
				{
					...
					$this->get_dependent_dropdown($sql);
				}
			}
		}
			...
			setTimeout(function(){
				...
				{ 
					...
					{ 
						...
						if (toolbar == 0) jQuery(el).after('<button>Calendar</button>').next().button({icons:{primary: 'ui-icon-calendar'}, text:false}).css({'font-size':'69%', 'margin-left':'2px'}).click(function(e){jQuery(el).datepicker('show');return false;});
		...
		function link_datetime_picker(el,fmt,toolbar,opts)
		{
			setTimeout(function(){
				...
				{ 
					...
					{
						opts = (typeof(opts) == 'undefined') ? {} : opts;
						if (toolbar == 0) jQuery(el).after('<button>Calendar</button>').next().button({icons:{primary: 'ui-icon-calendar'}, text:false}).css({'font-size':'69%', 'margin-left':'2px'}).click(function(e){jQuery(el).datetimepicker('show');return false;});
