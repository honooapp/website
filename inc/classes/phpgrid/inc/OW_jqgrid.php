 ...
 * @version 1.4.8 build 149-20130720
...
class jqgrid
{
				...
				foreach($rules as $key=>$val) 
				{
					...
					foreach($this->options["colModel"] as $link_c)
					{
						...
						if ($val['field'] == $link_c["name"] && !empty($link_c["formatoptions"]) && in_array($op, array("ne","eq","gt","ge","lt","le")))
						{
							...
							if ($link_c["formatoptions"]["newformat"] == "d/m/Y H:i:s")
							{
								$tmp = explode("/",$val['data']);
								$val['data'] = $tmp[1]."/".$tmp[0]."/".$tmp[2];
							}							
	 ...
	 */	
	function get_dependent_dropdown($sql, $firstblank = NULL)
	{
		$select = array();
	 ...
	 */	
	function get_dropdown_values($sql, $allblank = NULL)
	{
		if ($allblank)
		{
			return ":";
	...
	function set_options($options)
	{
		...
		{
			$show_form_nav = 'jQuery("#pData").show(); jQuery("#nData").show();';
		}
		else
		{
			$show_form_nav = 'jQuery("#pData").hide(); jQuery("#nData").hide();';
		}
		...
		{
			$this->internal["add_options"]["beforeShowForm"] .= '	// align dialog to center
																	var gid = formid.attr("id").replace("FrmGrid_","");
																	var dlgDiv = jQuery("#editmod" + gid);
																	...
																	dlgDiv[0].style.top = jQuery(parentDiv).position().top + Math.round((parentHeight-dlgHeight)/2) + "px";
																	dlgDiv[0].style.left = jQuery(parentDiv).position().left + Math.round((parentWidth-dlgWidth)/2) + "px";
																	if (typeof(onBeforeShowAddForm) == "function") { onBeforeShowAddForm(); }
																';	
			...
			$this->internal["edit_options"]["beforeShowForm"] .= '
																	...
																	dlgDiv[0].style.top = jQuery(parentDiv).position().top + Math.round((parentHeight-dlgHeight)/2) + "px";
																	dlgDiv[0].style.left = jQuery(parentDiv).position().left + Math.round((parentWidth-dlgWidth)/2) + "px";	
																	if (typeof(onBeforeShowEditForm) == "function") { onBeforeShowEditForm(); }
																';

			$this->internal["delete_options"]["beforeShowForm"] .= '
																	// align dialog to center
																	var gid = formid.attr("id").replace("DelTbl_","");
																	var dlgDiv = jQuery("#delmod" + gid);
																	var parentDiv = dlgDiv.parent(); // div#gbox_list
																	var dlgWidth = dlgDiv.width();
																	var parentWidth = parentDiv.width();
																	var dlgHeight = dlgDiv.height();
																	...
																	dlgDiv[0].style.top = jQuery(parentDiv).position().top + Math.round((parentHeight-dlgHeight)/2) + "px";
																	dlgDiv[0].style.left = jQuery(parentDiv).position().left + Math.round((parentWidth-dlgWidth)/2) + "px";	
																	if (typeof(onBeforeShowDeleteForm) == "function") { onBeforeShowDeleteForm(); }
																';

			$this->internal["view_options"]["beforeShowForm"] .= '
																	// align dialog to center
																	var gid = formid.attr("id").replace("ViewGrid_","");
																	var dlgDiv = jQuery("#viewmod" + gid);
																	var parentDiv = dlgDiv.parent(); // div#gbox_list
																	var dlgWidth = dlgDiv.width();
																	var parentWidth = parentDiv.width();
																	var dlgHeight = dlgDiv.height();
																	...
																	dlgDiv[0].style.top = jQuery(parentDiv).position().top + Math.round((parentHeight-dlgHeight)/2) + "px";
																	dlgDiv[0].style.left = jQuery(parentDiv).position().left + Math.round((parentWidth-dlgWidth)/2) + "px";
																	if (typeof(onBeforeShowViewForm) == "function") { onBeforeShowViewForm(); }
																';
	...
	function set_columns($cols = null)
	{
		...
		for($i=0;$i<count($cols);$i++)
		{
			...
			{
				$cols[$i]["formatter"] = "function(cellval,options,rowdata){ jQuery(document).data('wysiwyg_'+options.rowId,jQuery.jgrid.htmlEncode(cellval)); return jQuery.jgrid.htmlDecode(cellval); }";
				$cols[$i]["unformat"] = "function(cellval,options,rowdata){ return jQuery.jgrid.htmlDecode(jQuery(document).data('wysiwyg_'+options.rowId)); }";
			...
			{
				$sql = $cols[$i]["editoptions"]["onchange"]["sql"];
				$search_on = $cols[$i]["editoptions"]["onchange"]["search_on"];
				$field = $cols[$i]["editoptions"]["onchange"]["update_field"];
				$order = $cols[$i]["editoptions"]["onchange"]["order"];
				$firstblank = $cols[$i]["editoptions"]["onchange"]["firstblank"];
				$appendcustomjs = $cols[$i]["editoptions"]["onchange"]["appendcustomjs"];
				unset($cols[$i]["editoptions"]["onchange"]);
				if (!$appendcustomjs) {
					$cols[$i]["editoptions"]["onchange"] = "fx_get_dropdown(this,'$field');";
				}
				else {
					$cols[$i]["editoptions"]["onchange"] = "fx_get_dropdown(this,'$field'); {$appendcustomjs}";
				}
				...
				// on postback check
				if ($_POST["src"] == $cols[$i]["name"] && $_POST["return"] == "select")
				{
					$term = $_POST["value"];
					...
					if (!empty($term))
					{
						...
						$this->get_dependent_dropdown($sql, $firstblank);						
	...
	function render($grid_id)
	{
			...
			switch($op)
			{
				...
				case "clone":
						...
						if (!empty($this->events["on_clone"]))
						{
							...
							if ($obj)
								call_user_method($func,$obj,array($pk_field => $src_id, "params" => &$f));
							else
		...
		if (isset($_GET['jqgrid_page']))
		{
			...
			if (!empty($this->events["on_select"]))
			{
				...
				if ($obj)
					call_user_method($func,$obj,array("param"=> $_REQUEST, "grid"=>$this, "sql" => &$event_sql, "sql_count" => &$event_sql_count));
				else
				...
				if ($this->options["colModel"])
				{
					foreach ($this->options["colModel"] as $c)
					{
						...
						else
						{
							...
							$col_widths[$c["name"]] = (!empty($c["width"]) ? $c["width"] : '100');
						}
					}
				}
		...
		foreach($this->options["colModel"] as &$c)
		{
			...
			if (isset($c["editrules"]["readonly"]))
			{
				if ($c["editrules"]["readonly"] === true)
				{
					...
					$str_add_access .= 'jQuery("#tr_'.$c["index"].' .DataTD",formid).html("&nbsp;" + jQuery("#tr_'.$c["index"].' .DataTD '.$tag.'",formid).val());'; 
		...
		if ($this->actions["rowactions"] !== false)
		{
			...
			// 			for(var i=0;i < ids.length;i++){
			// 				var cl = ids[i];
			// 				be = ' <a title=\"Edit this row\" onclick=\"jQuery(\'#$grid_id\').editRow('+cl+',true); jQuery(this).parent().hide(); jQuery(this).parent().next().show(); \"><span class=\"ui-icon ui-icon-pencil\"></span></a>'; 
			// 				de = ' <a title=\"Delete this row\" onclick=\"jQuery(\'#$grid_id\').delRowData('+cl+'); \"><span class=\"ui-icon ui-icon-close\"></span></a>';
	...
	?>
		...
		jQuery("#<?php echo $grid_id?>").jqGrid('navButtonAdd',"#<?php echo $grid_id."_pager"?>",{caption:"Export",title:"Export", buttonicon :'ui-icon-extlink',
			onClickButton:function(){
				if ("<?php echo $this->options["url"]?>".indexOf("?") != -1)
					window.open("<?php echo $this->options["url"]?>" + "&export=1&jqgrid_page=1<?php echo $order_by?>");
				else
					window.open("<?php echo $this->options["url"]?>" + "?export=1&jqgrid_page=1<?php echo $order_by?>");
			} 
		});
		...
		// Export to pdf
		jQuery("#<?php echo $grid_id?>").jqGrid('navButtonAdd',"#<?php echo $grid_id."_pager"?>",{caption:"PDF",title:"Esporta i record filtrati in formato PDF", buttonicon :'ui-icon-extlink',
			onClickButton:function(){
				if ("<?php echo $this->options["url"]?>".indexOf("?") != -1)
					window.open("<?php echo $this->options["url"]?>" + "&export=1&jqgrid_page=1&export_type=pdf<?php echo $order_by?>");
				else
					window.open("<?php echo $this->options["url"]?>" + "?export=1&jqgrid_page=1&export_type=pdf<?php echo $order_by?>");
			} 
		});		
		...
		// Export to csv
		jQuery("#<?php echo $grid_id?>").jqGrid('navButtonAdd',"#<?php echo $grid_id."_pager"?>",{caption:"CSV",title:"Esporta i record filtrati in formato CSV", buttonicon :'ui-icon-extlink',
			onClickButton:function(){
				if ("<?php echo $this->options["url"]?>".indexOf("?") != -1)
			...
			setTimeout(function(){
				...
				{ 
					...
					{ 
						...
						if (toolbar == 0) jQuery(el).after('<button>Calendar</button>').next().button({icons:{primary: 'ui-icon-calendar'}, text:false}).css({'font-size':'100%', 'margin-left':'2px'}).click(function(e){jQuery(el).datepicker('show');return false;});
		...
		function link_datetime_picker(el,fmt,toolbar,opts)
		{
			setTimeout(function(){
				...
				{ 
					...
					{
						opts = (typeof(opts) == 'undefined') ? {} : opts;
						if (toolbar == 0) jQuery(el).after('<button>Calendar</button>').next().button({icons:{primary: 'ui-icon-calendar'}, text:false}).css({'font-size':'100%', 'margin-left':'2px'}).click(function(e){jQuery(el).datetimepicker('show');return false;});
		...
		function link_upload(el,field)
		{
			setTimeout(function(){
				...
				if(jQuery(el).val() != '')
				{
					...
					jQuery(el).parent().append("<input type='button' value='Delete' onclick='jQuery(\"#"+field+"\").val(\"\"); fx_replace_upload(\"#"+field+"\");' />");
					jQuery(el).hide();
				}
				else
				{
		...
		<?php if ($this->options["OWresizable"] === true) { ?>
		$(window).bind('resize', function () {
		    var oldWidth = jQuery("#<?php echo $grid_id?>").jqGrid("getGridParam", "width"),
		        newWidth = $(function_container).width();
		    if (oldWidth !== newWidth) {
		        jQuery("#<?php echo $grid_id?>").jqGrid("setGridWidth", newWidth);
		    }
		});
